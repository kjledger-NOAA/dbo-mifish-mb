...
#######  NOT UPDATED TO MATCH UPDATES FROM JUNE 2025 #############
...
```{r}
asv_table <- readRDS("/home/kimberly.ledger/dbo-mifish-mb/data/filtered.seqTab.RDS") %>%
  select(!Row.names)
...
asv_first <- which(colnames(asv_table_with_sample_type) == "ASV_1868")  #this is the first asv column in this dataframe
...
## Step 2. Consider tag-jumping using the positive controls 
...
asv_table_filter1 %>%
  ...
  labs(
    ...
    x = "sample ID",
    title = "taxon reads in positive controls") + 
 theme(
...
okay, some tag-jumping but will ignore this for preliminary analysis

## Step 3. Remove ASVs that do not occur/are less frequent in field samples
```{r}
...
more_in_positive <- reads_per_type_ASV %>%
  pivot_wider(names_from = "sample_type", values_from = c("TotalReadsPerASV")) %>%
  #filter(sample > 1) %>%
...
it will likely be useful to have a read count threshold (probably ~1000 reads per pcr replicate)
...
looks like sturgeon DNA was accidently added to NC35-B - or maybe swapped wit PC25-B? 
...
quite a few reads in a few extraction blanks  
...
e04253 and E2030.NC are the same extraction (abl vs pmel)
...
asv_table_filter1 %>%
  ...
  labs(
    y = "number of sequencing reads",
    x = "sample ID",
    title = "ASV reads in SKQ2021 samples") + 
  theme(
    axis.text.x = element_text(angle = 90, hjust = 0.95),
    legend.text = element_text(size = 8),
    legend.key.size = unit(0.3, "cm"),
...
asv_table_filter1 %>%
  ...
  labs(
    y = "number of sequencing reads",
    x = "sample ID",
    title = "ASV reads in SKQ2023 samples") + 
  theme(
    axis.text.x = element_text(angle = 90, hjust = 0.95),
    legend.text = element_text(size = 8),
    legend.key.size = unit(0.3, "cm"),
...
asv_table_filter1 %>%
  ...
  labs(
    y = "number of sequencing reads",
    x = "sample ID",
    title = "ASV reads in SKQ2024 samples") + 
  theme(
    axis.text.x = element_text(angle = 90, hjust = 0.95),
    legend.text = element_text(size = 8),
    legend.key.size = unit(0.3, "cm"),
...
asv_table_filter1 %>%
  ...
  labs(
    y = "number of sequencing reads",
    x = "sample ID",
    title = "ASV reads in NO20 samples") + 
  theme(
    axis.text.x = element_text(angle = 90, hjust = 0.95),
    legend.text = element_text(size = 8),
    legend.key.size = unit(0.3, "cm"),
...
asv_table_filter1 %>%
  left_join(metadata) %>%
  filter(cruise_id %in% c("DY2012", "DY2206", "DY2209", "DY2306")) %>%
  #filter(reads > 0) %>%
  filter(sample_type == "sample") %>%
  ggplot(aes(x=sample_ID, y=reads, fill=ASV)) +
  geom_bar(stat = "identity") + 
...
one thing i'm interested in is comparing the same extractions sequenced by ABL (in triplicate) and by PMEL (one pcr rep)

```{r}
skq23 <- asv_table_filter1 %>%
  left_join(metadata) %>%
  filter(cruise_id == "SKQ2023")

saveRDS(skq23, "/home/kimberly.ledger/dbo-mifish-mb/outputs/skq23_asv_table.RDS")
```
